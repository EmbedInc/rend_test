{   Subroutine REND_TEST_PERP_RIGHT (VPERP,MAG,V1,V2)
*
*   Make two vectors that are perpendicular to VPERP and to each other.  MAG
*   will be the magnitude of the two vectors.  V1xV2 will be in the direction
*   of VPERP.
}
module rend_test_PERP_RIGHT;
define rend_test_perp_right;
...
*   V1 is now perpendicular to VPERP.
  ...
  v2.x := (vperp.y * v1.z) - (vperp.z * v1.y); {make second perpendicular vector}
  v2.y := (vperp.z * v1.x) - (vperp.x * v1.z);
  v2.z := (vperp.x * v1.y) - (vperp.y * v1.x);
